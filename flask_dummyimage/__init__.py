...
@app.route('/dummyimage')
@app.route('/dummyimage/<string:size>')
def dummyimage(size='320'):
    # default variables
    font_path = os.path.join(os.path.dirname(__file__), 'fonts', 'DroidSans.ttf')
    font_size = 20

    # get args
    width, height = _get_size(size)
    text = request.args.get('text', "%dx%d" % (width, height))
    zoom_text = _get_bool(request.args.get('zoom', 'True'))
    draw_border = _get_bool(request.args.get('border', 'True'))
    debug = _get_bool(request.args.get('debug', 'False'))

    bg_color = _get_color(request.args.get('bg_color'), 'BG')
    text_color = _get_color(request.args.get('text_color'), 'TEXT')
    border_color = _get_color(request.args.get('border_color'), 'BORDER')
    ...
    ), text, font=font, fill=text_color)  # put the

    if debug:
        offset_width = (width - render_width) / 2
        offset_height = (height - render_height) / 2
        draw.rectangle(
            (0 + offset_width, 0 + offset_height,
             render_width + offset_width, render_height + offset_height),
            outline="red")
